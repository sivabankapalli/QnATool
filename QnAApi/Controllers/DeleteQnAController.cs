...
namespace QnAApi.Controllers
{
    ...
    [ApiController]
    public class DeleteQnAController : ControllerBase
    {
...
";*/
 ...
 ";*/
        ...
        async static Task<ResponseQnA> Patch(string uri, string body)
        {
            ...
            {
                var serializedItemToUpdate = JsonConvert.SerializeObject(body);
        ...
        async static Task<ResponseQnA> PostDeleteQnA(string kb, string new_kb)
        {
        ...
        async static Task<bool> DeleteQnA(string kb, string new_kb)
        {
            var response = await PostDeleteQnA(kb, new_kb);
        ...
        [HttpGet("{id}")]
        public async Task<string> UpdateQnA(int id,int qid,string qn,string ans)
        {
            string question = qn;
            string answer = ans;
            int qnid = qid;
            bool updateQnAResult = false;
            //string new_kb = @"{'add':{'qnaList':[]},'delete':{'ids':[" + qnid + "]},'update':{'qnaList':[]}}";
            string new_kb = @"{'add':{'qnaList':[]},'delete':{'ids':[" + qnid + "]},'update':{'qnaList':[]}}";
            ...
            using (var request = new HttpRequestMessage())
            {
                ...
                KBId = kbResult[id].Id;
                updateQnAResult = DeleteQnA(KBId, new_kb).Result;                
            }
